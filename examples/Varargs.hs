...
bldVarargs :: CodeGenModule (Function 'C (Word32 -> IO ()))
bldVarargs =
   withStringNul "Hello\n" (\fmt1 ->
   ...
   withStringNul "Two numbers %d %d\n" (\fmt3 -> do
      ...
      func <- createFunction ExternalLinkage $ \ x -> do
        ...
        let p1 = castVarArgs printf :: Function 'C (Ptr Word8 -> IO Word32)
        _ <- call p1 tmp1

        tmp2 <- getElementPtr0 fmt2 (0::Word32, ())
        let p2 = castVarArgs printf :: Function 'C (Ptr Word8 -> Word32 -> IO Word32)
        _ <- call p2 tmp2 x

        tmp3 <- getElementPtr0 fmt3 (0::Word32, ())
        let p3 = castVarArgs printf :: Function 'C (Ptr Word8 -> Word32 -> Word32 -> IO Word32)
        _ <- call p3 tmp3 x x
