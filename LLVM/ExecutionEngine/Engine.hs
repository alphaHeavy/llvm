...
withExecutionEngine :: MonadIO m
                    => ExecutionEngine
                    -> (Ptr FFI.ExecutionEngine -> IO a)
                    -> m a
withExecutionEngine ee ptr = liftIO $ withForeignPtr (fromExecutionEngine ee) ptr

-- |Create an execution engine for a module provider.
-- Warning, do not call this function more than once.
createExecutionEngine :: MonadIO m => Module -> m ExecutionEngine
createExecutionEngine m = liftIO $
  U.withModule m $ \mPtr ->
    alloca $ \eePtr ->
      alloca $ \errPtr -> do
...
destroyExecutionEngine :: MonadIO m => ExecutionEngine -> m ()
destroyExecutionEngine = liftIO . finalizeForeignPtr . fromExecutionEngine

touchExecutionEngine :: MonadIO m => ExecutionEngine -> m ()
touchExecutionEngine = liftIO . touchForeignPtr . fromExecutionEngine

runStaticConstructors :: MonadIO m => ExecutionEngine -> m ()
runStaticConstructors ee = liftIO $ withExecutionEngine ee FFI.runStaticConstructors

runStaticDestructors :: MonadIO m => ExecutionEngine -> m ()
runStaticDestructors ee = withExecutionEngine ee FFI.runStaticDestructors

getExecutionEngineTargetData :: MonadIO m => ExecutionEngine -> m FFI.TargetDataRef
...
-}
getPointerToFunction :: MonadIO m => ExecutionEngine -> Function f -> m (FunPtr f)
...
-}
addFunctionValue :: MonadIO m => ExecutionEngine -> Function f -> FunPtr f -> m ()
...
withAll ps a = go [] ps where
  go ptrs (x:xs) = withGenericValue x $ \ptr -> go (ptr:ptrs) xs
...
getRunFunction ee = withExecutionEngine ee $ \eePtr ->
  return $ \ func args ->
    withAll args $ \argLen argPtr ->
      createGenericValueWith $ FFI.runFunction eePtr func (fromIntegral argLen) argPtr
